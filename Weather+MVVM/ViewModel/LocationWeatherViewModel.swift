...
final class LocationWeatherViewModel {
    ...
    init(repository: SearchRepository, apiManager: APIService) {
        ...
        self.selectedCityInput.bind(nil) { [weak self] coord in
            if let coord {
    ...
    private func addCity(by location: CountryCoord) {
        apiManager?.fetch(to: FetchWeatherDTO(type: .forecast(lat: location.lat, lon: location.lon))) { [weak self] (data: ForecastCityResult?, error) in
            if let error {
                print(error.rawValue)
            }
            
            if let data {
                self?.repository?.addSearch(by: SearchModel(id: data.city.id, name: data.city.name, lat: data.city.coord.lat, lon: data.city.coord.lon))
            }
        }
    }
